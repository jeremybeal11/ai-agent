...
const functions_1 = require("../../app/api/chat/functions");
...
async function safeSigner(transactionInfo) {
    if (transactionInfo) {
        switch (transactionInfo.name) {
            case functions_1.TRANSACTION_COMMAND_NAMES.send:
                {
                    console.log("Sending funds to a wallet address...");
                    const walletAddress = transactionInfo.parameters.properties.address.value;
                    const amount = transactionInfo.parameters.properties.amount.value;
